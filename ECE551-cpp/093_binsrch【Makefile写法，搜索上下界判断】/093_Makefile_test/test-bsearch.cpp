...
template<typename T>
class OddFunction : public Function<int, int> {
	public:
		virtual int invoke(int arg){
			//this is the easiest way to do it.
			return 2 * arg - 1;
...
void check(Function<int,int> * f, int low, int high, int expected_ans, const char * mesg) {
	...
	int invoke_times = high > low ? log(high - low) / log(2) + 3 : 1;
...
int main() {
	...
	OddFunction<int> * of = new OddFunction<int>();
	...
	check(of, -3, 2, 0, "14 test case.  -3, 2, 0\n");
	printf("great! I know makefile!\n");
}